...
それ以降の`Start require (_document.js | _app.js)`では[キャシュを使用している](https://nodejs.org/api/modules.html#caching)のか、
モジュール解決は行われていません。
...
Node.jsは`require`関数は同期的、`import`関数は非同期的に実行します。
つまり、`Start require (_app.js | _document.js)`を実行したときに、`import`関数によるモジュール解決がマイクロタスクキューに積まれる可能性があります。
生成される`_app.js`を読むことはできなかったのですが、モジュール解決がマイクロタスクになっているとすると、上のログの間でモジュール解決が実行されているのも納得できます。
