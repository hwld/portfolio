...
JavaScriptエンジンはコードを実行する前に実行コンテキストを作成して、その中に変数や関数のためのメモリを割り当てます。
コードを実行する際には、実行コンテキストの中にある変数や関数を使用することになります。
コールスタックに積まれている実行コンテキストのうち、一番上にあるものは実行中の実行コンテキスト(Running execution context)と呼ばれます。

実行コンテキストは**スコープチェーン**というリストを持っており、これを使って識別子の解決を行います。
このスコープチェーンには、実行コンテキストが変数や関数のために割り当てるメモリ領域の参照を含むリストになっています。
他にも、現在の実行コンテキストから見える別の実行コンテキストのメモリ領域の参照も含んでいます。
コードを実行しているときに識別子を見つけると、現在の実行コンテキストのスコープチェーンのリストを確認して識別子を解決します。
...
- **グローバル実行コンテキスト**(GEC)
    ...
    - コンテキストには、グローバルに存在する変数や関数、Built-inのオブジェクトが保存される
- **関数実行コンテキスト**(FEC)
    - **ローカルコンテキスト**とも呼ばれる
    - 関数が呼び出されるたびに作成される
    - コンテキストには、その関数に存在する変数や関数が保存される

JavaScriptの実行によって、コールスタックに実行コンテキストが積み重ねられたり、取り出されたりします。
一番初めにグローバルコンテキストが作成されてコールスタックに積まれます。
コードが実行されて関数が呼び出される度にローカルコンテキストがコールスタックに積み上がっていき、関数から戻るとコールスタックから取り出されます。
これを繰り返していき、最終的にはコールスタックからグローバルコンテキストが取り出されて空になります。
...
外部環境APIに渡したコールバックは、**タスク**や**マイクロタスク**を作成して外部環境にあるキューに詰められ、適切なタイミングでコールスタックに積まれて実行されます。
コールバックを渡すことができる外部環境APIは、例えば`setTimeout`があります。
`setTimeout`は外部環境でタイマーを起動して一定時間後に、渡されたコールバック関数をタスクとしてタスクキューに詰めます。
...
先述しましたが、JavaScriptの実行開始によってもタスクはタスクキューに詰められます。
...
イベントループは外部環境ごとに違いはあるのですが、基本的には1つのループで以下のような処理を実行します。
...
イベントループは外部環境ごとに違いがあるのですが共通しているのは、「**単一のタスクを処理したあとに、すべてのマイクロタスクを処理する**」ということです。
...
フェーズは6つ存在しており、それぞれのフェーズで各タスクを実行したあと、すべてのマイクロタスクを事項します。
...
同時に実行できる数は、厳密にはCPUのコア数が上限になります。
...
JavaScriptの非同期処理は、外部環境APIとイベントループによって実現しています。
