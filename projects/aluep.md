...
Vercelにデプロイしており、起動に10秒以上かかるとタイムアウトでエラーになってしまいます。
...
Webアプリは、最初はCloud Runにデプロイしていましたが、コールドスタートの問題があって、現在はVercelにデプロイしています。
Vercelは無料枠では10秒を超えるとタイムアウトが発生してしまうのですが、Cloud Runは頑張ってもコールドスタート後の遅延が気になるのでVercelを選びました。

### コールドスタート対策

> [!warning]
> 追記:  
> 以降の対策は、調査する前の間違った前提に基づいて行っています。
> 静的ページにしても時間がかかる理由は、`_app.tsx`がバックエンドで読み込まれるからです。
> 調査の概要は[こちら](#追記-コールドスタートの調査概要)に書いています
...
2 ~ 7秒程かかっており、あまり効果がないように感じました。

> [!warning]
> 追記:  
> ホーム画面のFirst Load JSを減らしてもバックエンド側で読み込まれないため、あまり効果はありません。
> 上の対策によって`_app.tsx`も350kBから100kB程にはなっているのですが、React関連のモジュール解決に時間がかかってしまうので、
> `_app.tsx`を削ってもそこまで効果が感じられないのだと思います。
...
`server.ts`には静的ページの場合にはそのhtmlファイルをそのまま返すという実装はあります。
...
詳細な調査はできていませんが、今回のケースではReact関連のモジュールの解決に時間がかかっているように見えました。

### CI / CD環境

> [!warning]
> 以下はCloud Runにデプロイしていた頃の説明です。  
> 現在は簡単なコマンドでVercelにデプロイしています。
