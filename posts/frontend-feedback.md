...
ストレス要因は多岐にわたりますが、ここでは「**処理状況の不透明さ**」について考えていきます。
...
一方で、QueryやMutationによって自然にUIが変化しない場合には、トーストを使用して状態を伝えることができます。
...
そのためには、UIのパターンをただ覚えるだけでなく、ユーザーの視点に立って利用状況を理解し、最適なUIを適宜考えていくことが大切になります。
